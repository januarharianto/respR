...
#'
#' `calc.bg.rate` uses linear regression to calculate the rate of change of oxygen over time for background corrections of the main data. Can be used on multiple datasets of background measures, as long as the time data are identical between measurements.
#'
#' The output of `calc.bg.rate` can be fed as an argument directly into the functions [calc.rate] and [auto.rate].
#'
#' @author Januar Harianto & Nicholas Carey
#'
#' @md
#' @param df data frame. The data frame that contains the data needed for the function `calc.bg.rate`.
#' @param xcol numeric. The time column is specified here. Defaults to "`1`".
#' @param ycol numeric vector. The O2 column(s) are specified here. Defaults to "`2`".
...
#' \tabular{rll}{
#' \tab `lmfit`   \tab list of objects of class `lm`. There are all the regressions. \cr
#' \tab `results` \tab data frame. A summary of coefficients from each regression. \cr
