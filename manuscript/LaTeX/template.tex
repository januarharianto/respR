...
\usepackage[left=1.3in,top=1in,right=1.3in,bottom=1in]{geometry}
...
$if(anonymous)$\hfill $else$$for(author)$\textbf{\authorfont $author.name$} \hskip 8pt \emph{\small $author.affiliation$}  $sep$ \par $endfor$ $endif$
%$if(anonymous)$\hfill $else$$for(author)$\textbf{$author.names$} $sep$ \par $endfor$ $endif$
