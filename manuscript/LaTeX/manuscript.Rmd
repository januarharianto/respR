...
- name       : Maria Byrne
  affiliation: University of Sydney, School of Life & Environmental Sciences (SOLES), NSW Australia 2006
...
keywords: "R, software, respirometry, metabolic rate"
...
```{r Figure2, echo = F, fig.cap="Illustration showing the sampling method used by `auto.rate()` during **(A)** interval regression analysis, where sample windows do not overlap and **(B)** rolling regression analysis, where sample windows overlap and move forward by one sample unit at a time."}
...
pander(conversions, caption = "Units available for conversions in `calc.rate()` and `scale.rate()`.")
