...
<!-- FIGURE 1 -->
Here we describe the R package `respR`, a set of functions desgined to provide an efficient, structured, and reproduceable workflow for the analysis of respirometry-related data (**Table 1**). The package contains utilities to: (1) analyse closed, intermittent, flow-through and open respirometry data, (2) determine volume, mass, and surface area specific oxygen uptake rates, (3) automatically detect maximal and minimal rates, $P_{crit}$, or (4) detect the most linear sections of large datasets, with the help of traditional rolling regression and kernel density estimation techniques. Other functions useful in processing respirometry data are avaliable and are described in more detail in our online html vignette (https://januarharianto.github.io/respR/). The package has a strong focus on aquatic respirometry, however similar principals apply to aerial respirometry, so the analysis workflow should also be suitable for these data. We demonstrate the utility of the package by analysing real data that were collected using closed, intermittent and flow-through respirometry as proof of concept, and show how `respR` might be used to determine $MO_{2,max}$, $MO_{2,min}$, and $P_{crit}$. 
...
To determine $P_{crit}$, $RO_2$ (or $MO_2$) and ambient oxygen concentration ($PO_2$) must be provided. If $RO_2$ is not available, but the respirometry data exists to calculate it, `pcrit()` can automatically perform a rolling regression directly on the data to generate $RO_2$ and arrange it against the rolling mean of $PO_2$. This method is unique to `respR` -- in the past, a user may generate the same by cutting the data into equal sections and performing a linear regression on every section. A rolling regression 


## Converting units of $RO_2$ to $VO_2$ and $MO_2$

For delibrate reasons, units of measurements are not used in any of the functions `calc.rate.bg()`, `calc.rate()`, `calc.rate.ft()`, `auto.rate()` and `pcrit()`. Instead, two functions, `convert.rate()` and `scale.rate()`, need to be called separately to convert units and to perform volumetric or mass-specific scaling. This unique approach has the advantage of allowing us to perform "dimensionless" rate calculations for data that may not be fully supported by the entire `respR` workflow. For example, measurements of respiration can be scaled to surface area (diatoms, Iversen & Ploug, 2013, Naumann et al., 2014), volume (copepod eggs, Hammervold et al., 2015), or density (sea urchin larvae, Stumpp et al., 2011). Users may thus first perform rate calculations using `respR`, and then scale the results manually after if there is no support for their specific scaling method.

The function `convert.rate()` is used to convert units of oxygen concentration (e.g. mg/L or %) only and has support for over 132 unit combinations. As unit conventions differ between research disciplines and we do not expect 

`scale.rate()` directly accepts object outputs from prior rate-calculating functions and performs volumetric and mass-specific conversions. We have implemented a "semi-fuzzy" approach to 
...
While performing the required linear regressions to estimate the parameter of interest is a relatively simple task, a considerable amount of time is spent in data formatting and manipulation.
...
**Fig. 3** Typical graphical outputs of the function `auto.rate()`. (a) Depending on the `method` input, the function is able to show: (clockwise) the maximum rate detected, the minimum rate detected, calculation of rate by fixed-width intervals, and the most linear region as determined by kernel density estimates. (b) In addition, the outputs will also print diagnostic plots of: (clockwise) the rolling regression of rate as a function of time with dotted lines indicating the region used to estimate the parameter, a kernel density plot that shows only when the `linear` method is called, a residual plot of the regression, and a normal Q-Q plot of the residuals of the regression. All data is obtained from the example dataset `sardine` which is included in the package.
