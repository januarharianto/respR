...
3. We demonstrate how `respR` uses novel and parallelised computing methods such as rolling regressions and kernel density estimates to reliably detect maximum, minimum and most linear sections of the data, and critical oxygen tension, $P_{crit}$.
...
```{r}
inspect.data(squid, xcol = 2, ycol = 3, plot = FALSE, highlight = T)
```
