...
```{r echo = F, warning=FALSE, fig.alt = "inspect() output for intermittent-flow urchin data showing three replicates of oxygen over time with flush periods"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for first 1800 rows of intermittent-flow urchin data showing stable oxygen decline within first replicate"}
intermittent.rd |>
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for first urchin replicate with 50% rolling rate window showing consistently stable rate around -0.0006"}
intermittent.rd |>
  subset_data(from = 1, to = 1800, by = "row") |>
  inspect(width = 0.5)
```
...
```{r echo=FALSE, message=FALSE, fig.alt = "calc_rate.int() plot showing rates calculated across all three complete urchin replicates including flush periods"}
calc_rate.int(urchin_int,
...
```{r echo=FALSE, message=FALSE, fig.alt = "calc_rate.int() plot showing rates from specified measure phases of each urchin replicate excluding flush periods"}
calc_rate.int(urchin_int,
...
```{r echo=FALSE, message=FALSE, fig.alt = "calc_rate.int() plot showing rates from wait and measure phases of each urchin replicate with legend indicating data regions"}
urch_rates <-  calc_rate.int(urchin_int,
                             starts = c(1, 2101, 3901),
                             wait = 300,
                             measure = 900,
...
```{r message=FALSE, results='hide', fig.alt = "calc_rate.int() plot showing three urchin replicate rates in context of the full intermittent-flow dataset"}
plot(urch_rates,
     type = "full")
```
...
```{r message=FALSE, results='hide', fig.alt = "calc_rate.bg() output showing background oxygen consumption rate from control data"}
...
```{r echo=FALSE, message=FALSE, fig.alt = "auto_rate.int() output showing most linear rates extracted from each urchin replicate using linear method"}
urch_rates <-  auto_rate.int(urchin_int,
                             starts = c(1, 2101, 3901),
                             wait = 300,
                             measure = c(1500, 1100, 600),
