...
1. `check.data` checks for structural errors and select individual data from multi-column datasets.
...
4. `auto.rate` (optional) performs rolling regressions of a fixed width across all rows of the entire data frame, and can be used to obtain maximum and minimum rates.
...
4. `pcrit` to calculate the breakpoint in metabolic rate, based on Yeager and Ultsch (1989).


### `check.data`

`check.data` scans incoming data frames specific to timeseries and DO data for structural errors that may impede further analysis in R. Typically, a user may read data into R from a single spreadsheet file consisting of multiple columns of data. Invoking the function on a data frame perfoms a check on every column testing for data structure and common errors, which include: 
...
A user may not necessarily use `check.data` before running `calc.rate`, although it minimises errors and is useful as an exploratory tool before proceeding with the main analysis.

```{r, eval=F}
check.data(urchin2013)
```

`check.data` may also be used to subset two individual columns and produce a new data frame, e.g. `check.data(urchin2013, x = 1, y = 15)`. The user could use the function *directly* on a two-column dataset, and not specify the x and y arguments. As long as the first column is a timeseries, and the second column is DO data, the function will be able to successfully check the data for errors or warnings specific to their data types. If data successfully passes the checks, `check.data` produces a data frame object that can be saved and used in subsequenct functions, and also display a scatter plot of the data for visual inspection.


```{r, fig.width=4, fig.height=3}
# create subset for urchin #15 where column 1 is the time column and column 15 is the data for the urchin
urchin.n <- check.data(urchin2013, 1, 15)
```
