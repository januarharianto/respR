...
Some species are *oxyconformers*, with their routine oxygen uptake rate directly proportional to the oxygen supply available from the environment. So, for example, at 50% oxygen their uptake rate will be half that at 100% oxygen. Many species however are *oxyregulators*, and are able to regulate uptake and maintain routine rates as oxygen supply declines until a value is reached below which  uptake precipitously declines. Note however, the classification of species into strict oxyconformers and oxyregulators is somewhat simplistic; there is a continuum of responses between these extremes, and many intermediate cases [@muellerRegulationIndexNew2011].
...
Note how the rolling rate plot with the higher width is much smoother, with a less pronounced breakpoint. Increasing the width by too much can smooth out critical breakpoints, which is the very thing that the function is trying to detect. We can see the smaller width gives a similar result as the default, while the larger width estimates the breakpoints as a slightly higher value. Therefore, in this case the default value of 0.1 seems to be appropriate and give reliable results. 
...
Notice that we get identical critical values as the analysis on `oxygen~time` data [above](#bsr), despite the very different rate values (y-axis) and the fact these are *mass-specific* rates. Both absolute and mass-specific rates will give identical results.
