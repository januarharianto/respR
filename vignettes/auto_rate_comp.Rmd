...
As we can see, any dataset larger than around 400 to 500 in length takes a prohibitively long time to be processed by `rankLocReg`. A dataset of 5000 would take 47 days. In a test under the same conditions, `auto_rate()` processed a dataset of 5000 datapoints in size in 1.25 seconds. One dataset included in `respR` (`squid.rd`) is over 34,000 datapoints in length. Under the exponential relationship of `rankLocReg` this would take approximately *163 years* to run. `auto_rate()` completed analysis of this dataset in 18.5 seconds. 
