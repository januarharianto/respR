...
Because of `rankLocReg()`'s limitations, all simulated data are restricted to 150 data points. 
...
Because `rankLocReg()` uses three methods ("z", "eq", and "pc") to rank the data, we show (what we think is) the most appropriate method to rank the data.
We will let the plots speak for themselves; a more detailed perfomance analysis using `test_lin_data()` will be conducted once we complete modifications to the code to accept objects from `rankLocReg()`.
