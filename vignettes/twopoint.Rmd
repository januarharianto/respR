...
For some applications however, there may be valid reasons for using only two datapoints of oxygen concentration separated by a known time period to calculate a rate. This could be in periodic and irregular oxygen monitoring or sampling in the field, or alternatively where the investigator is not interested in oxygen uptake rates *per se*, but rather in total oxygen flux over a time period. An example of this is in respirometry studies of sediment communities, or where oxygen flux is correlated with transport of other chemicals or nutrients (e.g. Smith Jr. 1987). In such cases, even if continuous data exist, fitting a linear regression to these data may not give the correct information the user wants. This is particularly the case where oxygen or oxygen use fluctuates significantly over the time period of the experiment, such as with a diurnal rhythm. In such cases linear regressions may be significantly skewed.  

`respR` has the ability to calculate two-point rates. While we strongly recommend it not be used for analysis of data from any laboratory based studies, we have kept the functionality intact for the above use-cases, and for potential analysis of historical *closed-bottle* respirometry data. This vignette is here to show how it can be used. 
