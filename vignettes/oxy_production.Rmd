...
```{r, fig.alt = "inspect() output for algae oxygen production data showing raw timeseries and variable rolling rate over 20 hours"}
...
```{r results = "hide", fig.alt = "inspect() output for algae data with 50% rolling rate window showing highly stable production rate"}
...
```{r, fig.alt = "auto_rate() output for algae data showing most linear region and KDE analysis identifying stable production rate"}
...
```{r results='hide', fig.alt = "auto_rate() plot of second ranked result showing narrow low-rate region near end of algae dataset"}
...
```{r, fig.alt = "auto_rate() output for algae data with 50% width showing rate results and KDE analysis panels"}
...
```{r results='hide', fig.alt = "Overlap plot showing rate regions identified by auto_rate() with increased width across algae oxygen production dataset"}
