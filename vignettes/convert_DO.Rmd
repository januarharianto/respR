...
it, or use historical weather data to find out what it was on the day.
...
## Example 1 - Percent air saturation conversions
...
Where times have crossed midnight, our `lubridate` code has failed to parse it correctly. The lack of dates along with times means it has incorrectly calculated the elapsed time. 
...
Note that since `respR` is primarily used to examine oxygen consumption,
the rolling rate plot is by default plotted on a reverse y-axis. In `respR`
...
This better shows us the structure of this intermittent-flow experiment, and that within these replicates specimen rates appear to be consistent at around -0.002. 
...
= FALSE`. 
Console output messages can be suppressed with `message = FALSE` (in `plot()` only). 
Lastly, a different `width` value can be passed to see how it affects the rolling rate plot. 
