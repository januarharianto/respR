...
We also need to adjust the rate for background. Since it is the first replicate, we can simply use the starting background rate we calculated.
...
The MMR value we calculated earlier can also be passed to `convert_rate` to for conversion to actual units, to get the final mass-specific value for this specimen by entering the correct mass (of the fish) and volume. 
