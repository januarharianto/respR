...
## subset background recordings
  ...
  bg_start <- calc_rate.bg(bg_start)
  bg_end <- calc_rate.bg(bg_end)
...
## loop
## #NOTE - CHANGED TO 1:20 FROM 1:205 BECAUSE OF ERRO THAT HOPEFULLY FIXED NOW
for(i in 1:20){
...
## fit lm
bg_mod <- lm(c(bg_s$rate.bg, bg_e$rate.bg) ~ c(median(bg_s$data$Time), median(bg_e$data$Time)))
...
## ## #NOTE - CHANGED TO THIS BECAUSE OF ERROR THAT HOPEFULLY FIXED NOW
sr <- c(5000, seq(5840, 18000, 660))
...
z_ar_adj <- lapply(z_ar, 
                   function(g) adjust_rate(g, g$dataframe$x[1] * bg_mod_slp + bg_mod_int))
