...
In addition, the packages we compare here generally rely on complex date-time formats for the time metric, which we have found leads to substantial useability issues. 
...
Investigators working with respirometry are very much encouraged to explore `rMR` and `FishResp`. 
...
However, `respR` has been designed to be highly flexible, adaptable, and universal and with some basic `R` knowledge can be used for analysis of any aquatic respirometry data or experiment type we are aware of.
It also has statistically robust, linear-detection functionality which is not present in these packages.
We compare the performance of this to the only other `R` package which has similar functionality as seen [here](https://januarharianto.github.io/respR/news/auto_rate_comp.html). 
