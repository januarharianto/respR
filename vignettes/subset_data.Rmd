...
```{r results='hide', fig.alt = "inspect() output for squid.rd dataset showing full long-duration oxygen timeseries"}
inspect(squid.rd)
```

Let's say we are only interested in the initial stages. 

```{r results='hide', message=FALSE, fig.alt = "inspect() output for squid.rd data subset to first 10000 time units showing initial oxygen decline"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for squid.rd data subset to first 10000 rows"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for squid.rd data subset to oxygen values between 8 and 6.5 mg/L showing higher-oxygen portion"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for intermittent.rd dataset showing multi-replicate urchin oxygen timeseries"}
...
```{r message=FALSE, fig.alt = "auto_rate() output for a single replicate subset from intermittent.rd showing rate calculation via piped workflow"}
x |>
...
```{r message=FALSE, results='hide', fig.alt = "inspect() output for zeb_intermittent.rd data subset to time 10000 to 13000 for a quick visual check"}
zeb_intermittent.rd |>
