...
`calc_rate()` and `auto_rate()` - extract rates from the entire dataset or regions of it, manually or automatically

`adjust_rate()` - adjust the rate values for background oxygen use or production 

`convert_rate()` - convert the final adjusted rate to any common units of oxygen use or production
...
See `vignette("inspecting")` for a full description and example of what these checks entail, the implications of them failing or producing warnings, and other inputs in the function.
...
Entering the `inspect` object `urchin` that we saved earlier into `calc_rate()` with no additional arguments, will prompt the function to perform a linear regression on the entire data series. 
...
Based on the `from` and `to` arguments, a user may use `calc_rate()` to specify data ranges in several ways:
...
Conversion requires the units of the original data (`o2.unit`, `time.unit`), the `volume` of fluid in the respirometer (in **L**), for mass-specific rates the `mass` of the specimen (in **kg**), and for area-specific rates the `area` of the specimen (in **m^2**). Lastly, an `output.unit` should be specified. 
