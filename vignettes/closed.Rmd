...
We first use `inspect_data()` to prepare the data and to scan for:
...
**It should be noted that using `inspect_data` is optional** - the main functions in our package will readily accept any data frame as long as data are numeric and error-free. Running `inspect_data` is a qualitative, exploratory step that highlights potential issues about the data before analysis. We use this particular example, with an obvious error towards the end, to illustrate the point that you should always visualise and explore your data before analysis. `respR` has been designed to make this easy. 
...
Calling the function `calc_rate()` on the `inspect_data` object, with no additional arguments, will prompt the function to perform a linear regression on the entire data series. 

```{r}
calc_rate(urchin) # same as: calc_rate(urchin$df)
```

Note how the function recognises the `inspect_data()` object. Alternatively, you can specify a `data.frame` object containing raw data, in which case the function will automatically consider the first column as time data, and the second column as dissolved $O_2$ data.. 
...
The rate can be seen as the second entry `rate_b1`, and other summary data are saved in the object. The output also includes a `rate_2pt`. This is the rate determined by simple two-point calculation of difference in O~2~ divided by difference in Time. For almost all analyses, the `rate_b1` should be used. See the **"Two-point analyses"** vignette (LINK) for an explanation of this output and when it should be used. 
...
1. $VO_2$ - Total change in O~2~ per unit time within the chamber; or
2. $MO_2$ - Mass-specific rate of change in O~2~ per unit time of the specimen.
...
```{r results="hide", fig.keep="none", eval = FALSE}
## import and inspect
urchin <- inspect_data(urchins.rd, time = 1, oxygen = 15)
## background
...
urchin_MO2 <- convert_rate(a.rate, o2.unit = "mgl-1", time.unit = "m", output.unit = "mg/s/kg", 
    volume = 1.09, mass = 0.19)
```
