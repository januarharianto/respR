...
`respR` has a very simple structural data requirement: data must be in the form of paired values of numeric time and oxygen amount. Most if not all oxygen sensing equipment or probe systems output to this format, or at least to a format that is easy to coerce to this structure. All that is required is to put these data in a dataframe.
...
We can see this is essentially the same result as above. The small mismatch in values is simply due to the lower precision of entered values compared to internal ones (the number of decimal places printed depends on your own R options).
...
The final rate can be seen above in these console outputs. It can be extracted for further use from the saved object where it is `$rate.output`, or all results can be extracted using `$summary`. 

```{r}
urch_rate_final$rate.output
```

Alternatively, use the `summary` function with `export` to save the summary table as a separate data frame which contains all rate regression parameters and data locations, adjustments (if applied), units, and more. This is a great way of exporting all the relevant data for your final results. 
...
Dealing with so many different units and conversions it is incredibly easy to introduce a small conversion error that will have a massive effect on the final calculated rate. These types of error occasionally make it into published papers. This is why we would also recommend you check the result against more than one independent publication. 
...
Generally speaking, if there is a order of magnitude difference or more it is a good indication that there may be a problem. Here, we can see our rate is consistent with these others from the literature, and any difference is easily within what we might expect given variation between species and temperature. Therefore, we can be fairly confident we have not made a conversion error, and have found that our urchin rate is in the right ballpark.
