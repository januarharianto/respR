...
We can see this dataset contains time values, both outflow and inflow O₂ concentrations, and an O₂ delta, which is simply the inflow substracted from the outflow. The inflow (`o2.up`, which maintains a consistent mean value of 8.88) and O₂ delta columns are not required; we include them here to show different analyses later.  
...
We can change this by changing the column selectors. O₂ delta data can also be analysed to determine rates, as described later. 
```{r message=FALSE, warning=FALSE, fig.keep="none", results="hide"}
...
The `calc_rate.ft` function supports subsetting by row and time values, although this only works with data frames or objects of class `inspect_data`. We saw earlier how rates between rows 200 and 300 appeared to be most consistent. We will calculate rates from this data region based on both outflow O₂ values and O₂ delta values. 
