...
### Inspect data
Now we know the concentration of the header tank we can enter this as the `in.oxy.value` in `inspect.ft`, which will allow it to calculate an oxygen delta. We use `time` and `out.oxy` to specify the respective columns. 

```{r eval = F}
# inspect
insp1 <- inspect.ft(flowthrough.rd, time = 1, out.oxy = 2, in.oxy.value = 8.919)
```

```{r eval = T, echo = F}
# inspect
insp1 <- (suppressMessages(inspect.ft(flowthrough.rd, time = 1, out.oxy = 2, in.oxy.value = 8.919, plot = F)))
