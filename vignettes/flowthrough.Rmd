...
The `calc_rate.ft` function currently does not support subsetting. The dedicated `subset_data` function can be used to prepare data (see Reference menu above or `?subset_data`), or we can use straightforward R subsetting syntax as follows. We saw earlier how rates between rows 200 and 300 appeared to be most consistent, so we will calculate rates from this data region using the dataframe within the object we saved earlier. 
