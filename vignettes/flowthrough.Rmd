...
*"We want to calculate every rate of a fixed width across the entire dataset and then filter the results. We are interested in the routine metabolic rate from a particular region of the data."*

`calc_rate.ft` can calculate a rolling rate of fixed width in rows across the dataset. In this case there will be multiple rates in `output$rate` and the same number of rows in `output$summary`. The `subset_rate.ft()` function can be used to filter the results according to various criteria. See `vignette("subset_rate")` for examples with non-flowthrough data, but which are essentially the same.

Note the `calc_rate.ft` summary table contains linear regression coefficients alongside other metadata. These *should not be confused* with those returned by other functions such as `calc_rate` where slopes represent rates and coefficients such as a high r-squared are important. With flowthrough data the liner model is fit through the delta oxygen values averaged to provide a final rate. Therefore slope can represent the stability of the data region, in that the closer it is to zero, the less the delta oxygen values in that region vary, which is an indication of a region of stable rates. They are included to enable possible future functionality where stable regions may be automatically identified, and should generally be ignored. However, advanced users can use these and `subset_rate.ft()` to explore and subset the results if they wish. 
