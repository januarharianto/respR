...
Calculating a rate requires the `flowrate` of water through the respirometer, in a volume (`ul`, `ml`, or `L`) per unit time (`sec`, `min`, `hr`, or `day`). Here, only the value is required; the units will be entered in a later conversion function. For this dataset experimental data are in `?flowthrough.rd`, and in this example the flowrate is `2.34 mL/min`.
```{r eval = F}
# calculate rate
...
- As with `inspect()`, by using the `add.data` input an additional data source can be plotted in `inspect.ft` alongside the oxygen timeseries to help with understanding where rates may or may not have fluctuated. This is a visual aid only to help with selection of regions from which to extract rates. 
