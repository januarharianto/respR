...
```{r, fig.alt = "inspect() output for sardine data showing a successful check with no issues"}
inspect(sardine.rd)
```
...
```{r echo = F, eval = T, fig.alt = "inspect() output showing numeric check failure due to non-numeric time column"}
...
```{r echo = F, eval = T, fig.alt = "inspect() output after fixing non-numeric time values"}
insp <- suppressMessages(inspect(df))
```
...
```{r fig.show='hide', fig.alt = "inspect() output detecting Inf values in oxygen column"}
inspect(data)
```
...
```{r fig.show='hide', fig.alt = "inspect() output after replacing Inf values with interpolated data"}
...
```{r eval = T, fig.show='hide', fig.alt = "inspect() output showing non-sequential time values warning"}
...
```{r eval = T, fig.show='hide', fig.alt = "inspect() output after fixing non-sequential time values with format_time"}
...
```{r eval = T, echo = F, fig.show='hide', fig.alt = "inspect() output showing duplicate time values from row duplication"}
...
```{r fig.show='hide', fig.alt = "inspect() output after removing duplicate rows"}
...
```{r fig.show='hide', fig.alt = "inspect() output showing unevenly spaced time warning from decimalised minutes"}
...
```{r, echo = F, fig.alt = "inspect() output showing large time gap in data"}
insp <- suppressMessages(suppressWarnings(inspect(data)))
```
...
```{r results='hide', fig.alt = "calc_rate() plots showing rates extracted from the same dataset by row range and by time range producing equivalent results"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for sardine data with default 10 percent rolling rate width"}
## default width of 10%
inspect(sardine.rd)
```
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for sardine data with 20 percent rolling rate width showing smoother rates"}
## width of 20%
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for intermittent-flow data showing rate fluctuations at flush periods"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for a subset of intermittent-flow data showing a single replicate"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for a 22-hour zebrafish intermittent-flow experiment"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for a subset of the zebrafish experiment showing individual replicates"}
...
```{r results='hide', message=FALSE, warning=FALSE, fig.alt = "inspect() output for multiple oxygen columns of urchin data showing all timeseries"}
...
```{r , results = "hide", message=FALSE, warning=FALSE, fig.alt = "inspect() output for sardine data with temperature plotted alongside oxygen"}
...
```{r results='hide', message=FALSE, warning=FALSE, fig.alt = "inspect() output for algae oxygen production data with non-reversed rate axis"}
