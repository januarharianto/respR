...
This is what a successful `inspect` check looks like; all columns are numeric and without missing values, and the time data are sequential, unique values and with even spacing. 

```{r}
inspect(sardine.rd)
```

This dataset is therefore ready to be passed to further `respR` functions. While we have in general designed `respR` to be robust to data which might contain these issues, we have not been able to test every combination of them. Therefore, to be fully confident in results and avoid obscure errors we strongly recommend data be amended to pass these tests wherever possible. 
