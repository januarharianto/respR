...
This dataset is therefore ready to be passed to further `respR` functions. While we have in general designed `respR` to be robust to data which might contain these issues, we have not been able to test every eventuality. Therefore, to be fully confident in results and avoid obscure errors we strongly recommend data be amended to pass these tests wherever possible. 
