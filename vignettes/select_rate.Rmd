...
Here we will subset a portion of the `zeb_intermittent.rd` dataset, use `calc_rate.int` to extract a rate from the same time period of each, in this case from 3 minutes (180s) to 7 minutes (420s) within each replicate. Then we convert the results, plot them in two different ways, and show the summary table.
...
The first `"full"` plot shows the rate extracted from each replicate, but the second `"rate"` plot is really useful in understanding how rates change over the course of these 20 replicates. 
...
Getting the RMR is just as straightforward. Let's define it as the mean of the lowest 10th percentile of rates, so this time we pipe the result to `mean()`.
