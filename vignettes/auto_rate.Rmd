...
If we use `by = "time"` to determine rolling windows, `auto_rate()` automatically uses parallelisation techniques to speed up data processing. This may be disabled by setting the option `parallel = FALSE`.
...
We can customise the rolling window size and type by adding the `width` and `by` arguments: 
```{r}
