...
In addition, when used to detect linear sections of the data (Fig 1), a **kernel density estimate** is performed on the rolling regression output and the **kernel bandwidth** used to re-sample linear regions of the data for re-analysis. 

```{r, results="hide", fig.cap="Figure 1. Default output for the function `auto_rate()` when used on example data from `urchins.rd`. Automatic detection of the most linear segment of the data was performed."}
x <- urchins.rd[,c(1,15)]
auto_rate(x)
```
...
This is done by simply selecting all values in the rolling regression output that match the range of values around each mode of the KDE (i.e. $mode \pm h$).
Grouped rolling estimates are merged and the upper and lower bounds of each merged window are used to re-construct the segment of the data that they represent. Once all linear data segments are found, the rolling estimates are no longer needed and are thus discarded. The data segments are analysed using linear regression.
...
When used with `method = "linear"`, the `width` argument determines the minimum window size of the linear data obtained, but does not have any control over the maximum window size since that is automatically computed by KDE.
...
Olito et al. (2016) describes an different, and very robust, method to detect and rank linear methods in the data using their R package `LoLinR`. A comparison between `auto_rate()` and `LoLinR`'s performances are shown at the end of this document.
...
Thus to detect the right linear segment, `auto_rate()` will need to omit the initial non-linear segment, "identify" the dip, and then pick the longer of the 2 remaining linear segments that are separated by the dip.
Here we show 25 randomly-generated plots created by the method:
...
We repeated the tests using 100, 500 and 1000 data points.

```{r, eval = F}
# NOTE: functions may take a long time to run.
# this performs 1,000 iterations of auto_rate on a "default"-type data
set.seed(123)
default1k <- test_lin(reps = 1000, sd =.05, len = 300, type = "default", 
  preview = FALSE)
...
set.seed(456)
corrupted1k <- test_lin(reps = 1000, sd =.05, len = 300, type = "corrupted", 
  preview = FALSE)
...
set.seed(789)
segmented1k <- test_lin(reps = 1000, sd =.05, len = 300, type = "segmented", 
  preview = FALSE)
...
## Comparison of auto_rate() to other packages and software
