...
When you are looking for the lowest rates across a *specific time window*, representative of resting, basal or standard metabolic rates. 
...
When you are looking for the highest rates across a *specific time window*, representative of maximum or active metabolic rates. 
...
```{r rollreg_demo_2, fig.keep='none'}
# Perform linear detection; default width when not specified is 0.2:
normx <- auto_rate(sardine.rd)
...
A lower width leads to much more variable rolling rate estimates. Note how we have had to adjust the y-axis limits to fit the results (the left plot is the same data as in the previous section with different axis values). In this particular analysis (results not shown) the `linear` method was only able to identify 9 linear regions compared to 46 with the default width. If we were interested in highest or lowest rates, this would also prove problematic since the rates are so variable. 
...
See [Prinzing et al. 2021](https://onlinelibrary.wiley.com/doi/10.1002/ece3.7732) for an excellent discussion of appropriate widths in rolling regressions to determine maximum metabolic rates, much of which is relevant to extracting rates of any kind. 
...
**Users should take special note** that as a fully automated, unsupervised machine learning method of identifying linear data `auto_rate` is fallible, and the results should *always* be inspected and explored. 
...
Note, the output objects of the `highest` and `lowest` methods are virtually identical, the only difference being the results are ordered descending or ascending by rate, respectively. 
...
When using `auto_rate`, a plot of the results is produced (unless `plot = FALSE`). If there are multiple results each can be plotted individually using `pos`. Each panel can be plotted on its own using `choose` with values 1 to 6. If labels or legends obscure parts of the plot they can be suppressed using `legend = FALSE`. Console output messages can be suppressed with `quiet = TRUE`. Lastly, the rolling rate plot can be plotted on an unreversed y-axis with `rate.rev = FALSE`, if for instance you are examining oxygen production rates. 
