---
title: "Automatic detection of metabolic rates"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Automatic detection of metabolic rates}
...
This function can be used to select most linear, maximum and minimum rates in a statistically robust, objective way. 

In this vignette we detail how `auto_rate()` works, and how it can be used to extract rates from respirometry data. 
We show how we have [tested this method against simulated data](https://januarharianto.github.io/respR/articles/performance.html), and also [compare its performance against another linear detection method](https://januarharianto.github.io/respR/articles/auto_rate_comp.html). 
We show it perfoms extremely well on large datasets in determining linear rates. 
Importantly, `auto_rate()` has been optimised to be extremely fast. Other methods on large datasets can take minutes or hours to run.
...
This can be changed using the `width` argument, to a proportion relative to the total size of the data (`width = 0.1, by = "row"`) or a fixed value in the time metric (`width = 3000, by = "time"`). 
...
This represents the most consistently linear region, that is most consistent rate observed during the experiment. It does this in a statistically rigorous manner, which removes observer subjectivity from choosing which rate is most appropriate to report in their results. 
...
It is a statistically robust way of indentifying and reporting consistent rates in respirometry data. 
...
While we have had generally good results using `auto_rate()` on intermittent-flow data, there is a strong possibility flush periods could confuse the algorithms on occasion. See [intermittent-flow respirometry: complex example](https://januarharianto.github.io/respR/articles/intermittent2.html) for how to get around this. 
And as always, inspect outputs plots to ensure data locations are relevant. 
