...
```{r tidy = FALSE, results = "hide", fig.alt = "inspect() output and calc_rate() plots for urchin intermittent-flow data showing three replicate rate regions"}
# inspect data
...
```{r tidy = FALSE, results = "hide", fig.alt = "calc_rate() plots for urchin data showing rates extracted from different time windows in each of three replicates"}
urchin_int_rates <- calc_rate(urchin_int,
...
```{r results='hide', fig.alt = "calc_rate() plot for third urchin replicate showing rate extracted from specified time region"}
...
```{r eval = T, echo = F, results = "hide", fig.alt = "calc_rate() plot for third urchin replicate showing rate calculated from oxygen values 7.0 to 6.8"}
u_rate3 <- calc_rate(u_rep3, from = 7.0, to = 6.8, by = "oxygen")
```
...
```{r results="hide", message=FALSE, warning=FALSE, fig.alt = "inspect() output for zebrafish intermittent-flow data showing full timeseries and rolling oxygen uptake rate"}
...
```{r fig.alt = "Scatter plot of top-ranked zebrafish RMR rates from each replicate plotted on a reversed y-axis showing higher rates at experiment start"}
## extract rates
...
```{r message = FALSE, fig.alt = "Scatter plot of final converted zebrafish RMR rates across all replicates on reversed y-axis showing rate stabilisation after replicate 20"}
