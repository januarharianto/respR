...
```{r message=FALSE, warning=FALSE, results='hide', fig.alt="inspect output plot showing the complete intermittent-flow respirometry dataset with three replicates of oxygen over time and a rolling rate plot below"}
...
```{r results='hold', fig.alt="auto_rate.int plot showing the most linear rate extracted from each of three replicates using default settings with flush data included"}
...
```{r results='hold', fig.alt="auto_rate.int plot showing the most linear rate in each replicate with wait and measure phases highlighted and labelled, excluding flush data"}
auto_rate.int(urchin,
              starts = c(1, 2101, 3901),
              wait = 300,
...
```{r results='hold', fig.alt="auto_rate.int plot showing the top three ranked linear rates extracted from each replicate with wait and measure phases labelled"}
auto_rate.int(urchin,
              starts = c(1, 2101, 3901),
              wait = 300,
              measure = c(1500, 1100, 600),
...
```{r results='hold', fig.alt="auto_rate.int plot showing the lowest rate over an 8-minute window extracted from each of three replicates using time-based selection"}
auto_rate.int(urchin,
              starts = c(0, 35, 65),
              wait = 5,
              measure = c(25, 15, 15),
...
```{r results='hold', fig.alt="auto_rate.int plot showing the highest rate over an 8-minute window extracted from each of three replicates using time-based selection"}
auto_rate.int(urchin,
              starts = c(0, 35, 65),
              wait = 5,
              measure = c(25, 15, 15),
...
```{r results='hide', fig.keep='last', message=FALSE, fig.alt="auto_rate.int plot showing the lowest 3-minute rate from the first three and last three replicates of a zebrafish intermittent-flow experiment with 105 regularly spaced replicates"}
...
```{r results='hide', fig.alt="auto_rate.int plot showing rate results for replicates 50 to 69 of the zebrafish intermittent-flow experiment"}
...
```{r results='hide', fig.alt="auto_rate.int replicate-level plot showing each replicate individually with rate region highlighted and wait and measure phases shaded and labelled"}
...
```{r results='hide', fig.alt="auto_rate.int full dataset plot showing each replicate rate highlighted in the context of the entire urchin oxygen timeseries"}
auto_rate.int(urchin,
              starts = c(1, 2101, 3901),
              wait = 300,
              measure = c(1500, 1100, 600),
...
```{r results='hide', fig.alt="auto_rate.int full dataset plot showing replicate 50 rate result highlighted in the context of the entire zebrafish dataset"}
...
```{r results='hide', fig.alt="auto_rate diagnostic plot for replicate 50 showing the auto_rate result with rolling rate, residuals, and fitted rate within the measure phase"}
