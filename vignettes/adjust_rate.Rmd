...
plot.temp <- function(x, y, width = NULL, pos = NULL, message = TRUE,
  ...
  if (length(dt) == 2) {
    ...
    ylim <- range(nainf.omit(dt[[2]]), nainf.omit(dty[[2]]))
...
We use the interlinked `respR` functions of `inspect`, `calc_rate`, `calc_rate.bg` etc. in the following examples, but for most of these data frames or numeric values can also be used. We'll show some examples of these below. 
...
Note that in general the background recording does not have to be conducted at the same time as experiments, or even over the same duration as the experimental chambers to be adjusted. While this is ideal practice, it's not always practical. Its purpose it to establish a general background oxygen change per unit time relationship to be applied to specimen experiments of any length. 
...
Or, enter the five background rates directly as a numeric vector and let the default `method = "mean"` be applied.
...
This method will not accept numeric inputs. The `"concurrent"` method is fairly straightforward. Given a `calc_rate` or `auto_rate` object as input, for each rate contained in it, `adjust_rate` uses the regression parameters from the `$summary` table to fit a regression over the same region in the background data entered as the `by` input, the slope of which is the background rate. The `by` input can be a `data.frame`, `inspect` or `calc_rate.bg` object. If one of the latter two, the function uses only the `$dataframe` element. If there is more than one column of oxygen in the background data, the function calculates the background rate across the same region in *all* columns, and the mean of these are applied as the adjustment. Therefore, concurrent rates from single or multiple controls can be used to adjust specimen rates. 
...
sqd_bg[[3]] <- convert_DO(sqd_bg[[2]], "%Air", "mg/l", t = 15, S = 30, P = 1.01)$output
sqd_bg <- sqd_bg[,c(1,3)]
sqd_bg[[1]] <- 0:7512
```
...
We would expect the background rate to be lower at all stages of an experiment except the very end under an exponential vs. a linear relationship. Here, we do see slightly lower adjustment values than in the linear example [above](#lineareg2). 
