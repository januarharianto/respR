...
After calculating background rates from a control or "blank" experiment, the function can adjust rates from `calc_rate`, `calc_rate.int`, `auto_rate`, `auto_rate.int`, or numeric values in the following ways:
...
In the `respR` workflow, `adjust_rate` should be used on rates which have been determined in `calc_rate`, `calc_rate.int`, `auto_rate`, or `auto_rate.int` or rate values (change in oxygen units per unit time) otherwise determined from the raw data, but before they are converted in `convert_rate`. 
...
`calc_rate`    - calculate rates from *single or multiple* regions of a *single* oxygen column.
...
```{r echo=FALSE, warning=FALSE, message=FALSE, fig.keep=2, results='hold', fig.alt="Inspect plot of background oxygen data from urchins column 18 showing oxygen timeseries and rolling rate, followed by background rate regression fitted across the full timeseries."}
## inspect and calculate background rate using calc_rate.bg
bg <- inspect(urchins.rd, time = 1, oxygen = 18) |>
...
```{r echo = FALSE, warning=FALSE, message=FALSE, fig.keep=1, fig.alt="Plot of calc_rate result showing the fitted linear regression over the selected time region of urchin oxygen data between 10 and 30 minutes."}
## inspect and calculate urchin rate using calc_rate
...
```{r warning=FALSE, message=FALSE, results='hide', fig.keep=2, fig.alt="Inspect plot of two background oxygen columns 18 and 19 from the urchins dataset showing oxygen timeseries and rolling rates for both columns."}
...
```{r message = FALSE, results='hide', fig.alt="Plot of calc_rate.bg results showing background rate regressions fitted to three oxygen columns from a separate background experiment."}
...
```{r results='hide', message=FALSE, fig.alt="Inspect plot of squid respirometry data showing oxygen timeseries and rolling rate over time."}
sqd_insp <- inspect(sqd)
```

#### Prepare background data

The background data is in a separate data frame, `sqd_bg`. There is no need to calculate a rate; `adjust_rate` will do this when it identifies the relevant data regions (although it will accept `calc_rate.bg` objects). We could also use a data frame as the `by` input. The only requirement is that the time is in column 1 and all other columns are the oxygen columns you want to use to calculate the concurrent background rate. However it's always a good idea to inspect the data for issues and visualise it using `inspect`, and this function also allows us to extract the columns we are interested in. 

```{r results='hide', message=FALSE, fig.alt="Inspect plot of squid background data showing oxygen timeseries and rolling rate, with the rolling rate plot revealing an increasing background rate over time."}
...
```{r message = FALSE, results='hide', fig.alt="Plot of auto_rate results on squid data showing detected linear regions highlighted on the full timeseries and individual rate regressions."}
...
```{r message=FALSE, results='hide', fig.show='asis', fig.alt=c("Plot of calc_rate.bg result showing background rate regression fitted to the subset of background data from rows 2425 to 4800.", "Plot of calc_rate.bg result showing background rate regression fitted to the subset of background data from rows 273 to 3057.")}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.keep='last', results='hide', fig.alt="Plot showing intermittent-flow respirometry data with specimen data in black and concurrent control data in orange, displaying repeated flush-measure cycles."}
interm <- zeb_intermittent.rd[16290:18929,]
...
interm_bg[[3]] <- interm_bg[[3]] + runif(2640, -0.04, 0.04)
...
The replicates follow a specific cycle of 9 minutes closed, two minutes flushing. Knowing this, we can use `calc_rate.int` to extract a rate from the same time window within each using the `wait` and `measure` inputs. We use a `wait` of five minutes (300 rows) at the start of each, which excludes the flush and start of the replicate data. We then use a `measure` phase of six minutes (360 rows) to extract a rate from the rest of the replicate.

```{r message=FALSE, results='hide', fig.alt="Plot of calc_rate.int results showing extracted rates from each replicate of the intermittent-flow experiment with wait and measure phases indicated."}
...
If you want to check the adjustment is correct, and that the times used match those of each rate, these are found in the `$adjustment_model` element of the output object.
...
```{r results='hide', message=FALSE, warning=FALSE, fig.keep=c(2,4), fig.alt=c("Inspect plot of urchin oxygen data from column 2 showing full timeseries with rolling rate.", "Plot of calc_rate result showing the fitted linear regression for the urchin rate between 10 and 30 minutes.")}
...
```{r results='hide', message=FALSE, warning=FALSE, fig.keep=1, fig.alt="Inspect plot of two background oxygen columns 18 and 19 from the urchins dataset showing oxygen timeseries for both columns."}
...
```{r message = FALSE, results ='hide', fig.keep=c(2,4), fig.alt=c("Plot of calc_rate.bg result showing the background rate regression fitted to pre-experiment control data from 1 to 4999 seconds.", "Plot of calc_rate.bg result showing the background rate regression fitted to post-experiment control data from 75140 to 79251 seconds.")}
# pre
...
```{r message=FALSE, warning=FALSE, results="hide", fig.keep='last', fig.alt="Plot of auto_rate results on a single zebrafish replicate showing detected linear regions and their fitted regressions."}
...
Now we'll adjust the specimen rates using the two background rates and the `"linear"` method. 
...
The `background_exp.rd` example dataset shows background data with a rate that increases exponentially. 

```{r results='hide', message=FALSE, fig.alt="Inspect plot of exponential background data showing oxygen timeseries and rolling rate, with the rolling rate plot revealing an exponentially increasing background rate over time."}
...
```{r results='hide', message=FALSE, warning=FALSE, fig.alt="Plot of calc_rate result on algae data showing a positive slope indicating oxygen production via photosynthesis."}
...
```{r results='hide', message=FALSE, warning=FALSE, fig.alt="Plot of calc_rate.bg result showing the background rate regression fitted to the algae control experiment data with a negative slope indicating oxygen consumption."}
...
```{r message=FALSE, warning=FALSE, results='hide', fig.alt = "inspect() output for urchin data showing specimen oxygen uptake channel and background channel timeseries"}
...
```{r results='hide', message=FALSE, fig.alt = "inspect() output for background data showing oxygen timeseries with initial spike before stable background measurement region"}
inspect(bg_data)
```
...
```{r message=FALSE, results='hide', fig.alt = "calc_rate.bg() output showing background oxygen consumption rate calculated from stable region of background data after row 5000"}
...
If you just want to extract a single rate, using `calc_rate` and its region selection inputs to calculate a background rate from a specific region is also an option. See [here](#crbgvcr) for discussion about the main differences between `calc_rate.bg` and `calc_rate`.
