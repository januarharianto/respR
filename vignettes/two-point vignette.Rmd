...
The two-point rate can be seen at the end of the summary object. With only two datapoints, it will necessarily be equal to the rate in the linear regression method. However, it can be extracted directly:
...
Here we can see there are two erroneous regions. We could remove these sections, or use the lower section of the data, but we can also use the subsetting options of `calc_rate` to select data that starts and ends where we know valid O₂ recordings were made:
...
```{r}
adjust_rate(two_pt_result$rate_2pt, -0.01)
