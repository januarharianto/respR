...
As well as `pcrit` being removed, `calc_pcrit` has been renamed `oxy_crit()` to avoid conflicts with another package. 
...
Several function inputs have been renamed to make them more consistent. 
 ...
 - All primary inputs are now `x` (was previously `df` in `auto_rate`, `inspect`, and `subsample` among others). Any code using `df =` can be replaced with `x =`. 
