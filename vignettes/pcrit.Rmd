...
For now, we can see the dataset is free of errors or issues, so we can pass the saved `market.squid` object to the dedicated `calc_pcrit()` function. 

### $P_{crit}$ function

The `calc_pcrit()` function provides three methods to report the breakpoint in the rate ~ O~2~ relationship. 
...
**Note**, that the `calc_pcrit` function has a `width` input, which controls the width of the rolling regressions used to calculate the rolling rate values.
...
The `calc_pcrit()` function also includes a default `parallel = TRUE` argument.
...
Note, the `calc_pcrit()` function does not support adjusting rates for background effects.
...
For now, users should be aware that rates extracted from objects of class `calc_pcrit` will not have been background-corrected.


### Extracting results

Results of `calc_pcrit()` analyses can be extracted from the results object via the following:
...
The `segmented` method included in the `calc_pcrit()` function in `respR` is simply a wrapper for the [`segmented`](https://cran.r-project.org/web/packages/segmented/) package by Muggeo (2008). 
