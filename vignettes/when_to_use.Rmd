...
In our opinion, you should use `respR` if you have aquatic respirometry data. End of. We see no real benefits of using other options. Admittedly, for obvious practical reasons, we have not processed every data format or variations of experiment, so if for whatever reason some part of `respR` does not work for you let us know (CONTACT LINK), and we will try to accomodate it.  
