...
For files types that are not yet supported, or if you have already imported your data by other means, the `format_time()` function can parse date-time columns to numeric time-elapsed values, if the imported file does not already contain this. Internally, it uses functionality in the package `lubridate`. 
