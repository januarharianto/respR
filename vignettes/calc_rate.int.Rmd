...
```{r message=FALSE, warning=FALSE, results='hide', fig.alt="inspect output plot showing the complete intermittent-flow urchin dataset with three replicates of oxygen over time and a rolling rate plot below"}
...
```{r tidy=FALSE, results='hide', fig.alt="calc_rate.int plot showing rates calculated across entire replicates including flush periods"}
...
```{r tidy=FALSE, results='hide', fig.alt="calc_rate.int plot showing rates from each replicate with different measure phases excluding flush periods"}
calc_rate.int(urchin,
              starts = c(1, 2101, 3901),
...
```{r tidy=FALSE, results='hide', fig.alt="calc_rate.int plot showing rates from a standardised region within each replicate using wait and measure phases to exclude flush and settling periods"}
calc_rate.int(urchin,
              starts = c(1, 2101, 3901),
              wait = 180,
              measure = 600)
...
```{r results='hide', fig.alt="calc_rate.int plot showing rates from each replicate using time-based selection of wait and measure phases"}
calc_rate.int(urchin,
              starts = c(0,35,64),
              wait = 3,
              measure = 10,
...
```{r results='hide', fig.keep='last', message=FALSE, fig.alt="calc_rate.int plot showing rates from the first three and last three replicates of a zebrafish intermittent-flow experiment with regularly spaced replicates"}
...
```{r results='hide', fig.alt = "calc_rate.int() plot showing replicates 50 to 69 of zebrafish intermittent-flow experiment on a grid"}
...
```{r results='hide', fig.alt = "calc_rate.int() plot type rep showing three urchin replicates individually with time-based rate regions highlighted"}
calc_rate.int(urchin,
              starts = c(0,35,64),
              wait = 5,
              measure = 15,
...
```{r results='hide', fig.alt = "calc_rate.int() plot type full showing three urchin replicate rates highlighted in context of the complete dataset"}
calc_rate.int(urchin,
              starts = c(0,35,64),
              wait = 5,
              measure = 15,
...
```{r results='hide', fig.alt = "calc_rate.int() plot showing zebrafish replicate 50 rate highlighted in context of the full dataset"}
...
```{r results='hide', fig.alt = "calc_rate() plot for zebrafish replicate 50 showing the individual rate regression within the replicate subset"}
