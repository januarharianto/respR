...
The example data (`zeb_intermittent.rd`) was kindly provided by Dr. Davide Thambithurai (University of Glasgow), and is from an experiment on a zebrafish. It is comprised of one replicate of 14 minutes duration (12 minutes recording, 2 minutes flushing), followed by 105 replicates of 11 minutes duration (9 minutes recording, 2 minutes flushing). See `?zeb_intermittent.rd` for row locations of all stages. 

See [here]() for an example with a smaller dataset, which covers in more detail the basics of analysing intermittent-flow data, and is a better starting point if you are new to the package. Even if not, we would recommend reading both before starting your analysis. 
...
bg_start <- subset_data(zeb, from = 0, to = 4999, by = "time") |>
  calc_rate.bg()

bg_end <- subset_data(zeb, from = 75140, to = 79251, by = "time") |>
  calc_rate.bg()
```

The two different background rates (only the end one is plotted above) have been saved to these two objects.
...
We can see the background rate increasing by around 70% over the course of the experiment. We will use these later in [Adjusting rates](#adjust) to apply a dynamic background adjustment to rates based on their time during the experiment.
...
These are all aspects that may feed into rate selection criteria regarding a specific experiment or study, and it depends on these whether this in an acceptable, reportable result. Maybe at around 3 minutes, this is not a long enough duration for us to be comfortable that this was a sustained MMR. In which case maybe the first result is better reported. Or maybe we could report an average of all the rates from this `auto_rate` analysis or a selection of them, such as the top three. 
...
High width values can lead to overfitting and mis-estimation of rates. See [here]() for discussion of this in the context of `auto_rate`, and again we recommend reading [Prinzing et al. 2021](https://onlinelibrary.wiley.com/doi/10.1002/ece3.7732) to understand the nuances of determining MMR. See also [Killen et al. 2021](https://doi.org/10.32942/osf.io/gnzh7). 
...
Calculating RMR is a only a little more complex than we did above in [MMR](#mmr). Manually subsetting every replicate would be difficult, but since we know they cycle at 11 minutes (660s) and there are 105 further replicates after the first ends at row 5839, we can use straightforward R syntax. 
...
We can extract and view the primary result from each replicate using the `sapply` function.
...
Let's look at two results, one from early in the experiment and one from later, and we will use the `pos` input to select only the top ranked result.
```{r}
...
zeb_mmr_conv <- convert_rate(zeb_mmr_adj,
                             o2.unit = "mg/L",       # oxygen units of the original data
                             time.unit = "secs",     # time units of the original data
                             output.unit = "mg/h/g", 
...
```{r results='hide'}
mmr_data <- summary(zeb_mmr_adj, pos = 6, export = TRUE)
```
...
Therefore, we will not use this one, but take the mean of all others from 20 onwards. This is just one approach. A different approach is in the next example for SMR.
