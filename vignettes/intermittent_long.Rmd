...
```{r results="hide", message=FALSE, warning=FALSE, fig.alt = "inspect() output for initial 18000 rows of zebrafish intermittent-flow data showing start of experiment"}
zeb_intermittent.rd |>
...
```{r results="hide", message=FALSE, warning=FALSE, fig.alt = "inspect() output for end section of zebrafish intermittent-flow data showing post-experiment state"}
zeb_intermittent.rd |>
...
```{r results="hide", message=FALSE, warning=FALSE, fig.alt = "inspect() output for complete zebrafish intermittent-flow dataset showing full oxygen timeseries"}
...
```{r fig.keep="last", message=FALSE, warning=FALSE, results="hide", fig.alt = "calc_rate.bg() output showing post-experiment background oxygen consumption rate from zebrafish data"}
bg_pre <- zeb |>
...
```{r message=FALSE, warning=FALSE, results="hide", fig.show='all', fig.alt = "inspect() plot of first zebrafish replicate showing declining oxygen uptake rate over the measurement period"}
# subset rep 1
...
```{r fig.alt = "calc_rate() plot showing five-minute measure phase rate from zebrafish MMR replicate after three-minute wait"}
...
```{r fig.alt = "auto_rate() highest method output showing highest two-minute oxygen uptake rates from zebrafish MMR replicate"}
zeb_mmr <- zeb_rep_1 |>
...
```{r results = "hide", fig.alt = "auto_rate() linear method plot of fifth ranked result showing highest rate region in zebrafish MMR replicate"}
plot(zeb_mmr, pos = 5)
```
...
```{r results = "hide", fig.alt = "auto_rate() linear method plot of fifth ranked result with increased width showing smoother rolling rate for zebrafish MMR"}
plot(zeb_mmr, pos = 5)
```
...
`calc_rate.int` was introduced in [`respR v2.1`](https://januarharianto.github.io/respR/articles/release_notes.html#version-2-1-2022-12-06) and allows you to specify a replicate structure and run `calc_rate` on each replicate to extract a rate from the same time or row region in each. See `vignette("calc_rate.int")` for details of the functionality. 
...
```{r results='hide', fig.keep='last', fig.alt = "inspect() output for subsetted zebrafish intermittent-flow data showing 105 replicates and rolling oxygen uptake rate"}
...
```{r results='hide', fig.alt = "calc_rate.int() output showing first 20 zebrafish replicates with wait and measure phases highlighted and extracted rates"}
...
As of [`respR v2.1`](https://januarharianto.github.io/respR/articles/release_notes.html#version-2-1-2022-12-06) `convert_rate` has new plotting functionality to help visualise and explore rate results. For intermittent-flow results the most useful of these is `type = "rate"`. 

```{r message = FALSE, results='hide', fig.alt = "Rate plot of calc_rate.int converted results showing zebrafish oxygen uptake rates across all replicates over time"}
...
```{r results='hide', fig.alt = "auto_rate.int() output showing lowest two-minute rate extracted from each zebrafish replicate with measure phase highlighted"}
...
```{r message = FALSE, results='hide', fig.alt = "Rate plot of auto_rate.int converted results showing zebrafish SMR-candidate oxygen uptake rates across all replicates"}
...
```{r message = FALSE, results='hide', fig.alt = "Rate plot of zebrafish auto_rate.int results for replicates 30-80 showing anomalously low rate in replicate 48"}
...
```{r message = FALSE, results='hide', fig.alt = "Diagnostic plots for zebrafish replicate 48 showing anomalous oxygen trace and auto_rate analysis output for that replicate"}
