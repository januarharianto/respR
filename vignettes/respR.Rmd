...
`respR` was designed to work best with data frame objects, including objects of class `data.frame`, `data.table` and `tibble`, containing numeric time and oxygen data. The function `format_time` (see **Reference** section above) will help you prepare your time data if it is not in numeric form. It can automatically convert many date-time formats data to numeric time.
