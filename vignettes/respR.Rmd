...
Note that the user does not need to be overly precise; for input values of O~2~ and time that do not match exactly to a value in the data, the function will identify the closest matching time or O~2~ values to the inputs, rounded down, and uses these for subsequent calculations.
...
summary(rate)
```
Note, the output also includes a `rate_twopoint` output. This is the rate determined by simple two-point calculation of difference in O~2~ divided by difference in Time, and can be useful in determining rate where only a limited number of data points are available, such as in 'closed bottle' respirometry. 
...
Since background rate has been calculated in `calc_rate.bg()`, adjustment is straightforward using the function `adjust_rate`. 
...
For example, the user may convert the output of `calc_rate()` to O~2~ consumed per hour:
